 ...
 * FS-04/05/06 now hangs off. This module owns its lifecycle outside the bracket
 * (playoffs.ts):
 *   - ensureSeason   — called on draft.complete: create/activate the season the
 *                      schedule + scores attach to (idempotent).
 ...
 *   - read helpers    — list past seasons and one season's final standings +
 *                       bracket for the read-only history endpoints.
 *
 * The `season SMALLINT` columns keep human-readable numbering; season_id FKs
 * them to the row this creates (see migration 1700000000019).
 */
...
const SEASON_COLS = `id, league_id, season_number, status, regular_weeks,
        playoff_seeds, champion_user_id, started_at, ended_at`;
...
/** Default playoff field size for a league — top 4, clamped to its size. */
function defaultSeeds(size: number): number {
...
/**
 * Create (or activate) the league's current season — the one the schedule and
 * scores attach to. Called in-process on draft.complete, post-commit. Activates
 * the latest non-archived season (the one startNewSeason opened for a re-draft),
 * or opens season 1 the first time. Idempotent: a re-fired draft.complete just
 * re-stamps started_at. Returns the season row.
 */
export async function ensureSeason(
  ...
  const { rows: lg } = await db.query<{
    ...
    size: number;
  }>(`SELECT season_length_weeks, size FROM fs_league WHERE id = $1`, [
    leagueId,
  ]);
 ...
 * season to be archived (the bracket crowned a champion). Increments
 * season_number, clears the prior roster + draft so the league can re-draft,
 * and returns the league to `forming`. Membership and every prior season's
 * matchups/scores/standings are preserved (they key on the old season number).
 */
export async function startNewSeason(
  ...
  try {
    ...
    const { rows: lg } = await client.query<{
      ...
      `SELECT season_length_weeks, size FROM fs_league WHERE id = $1 FOR UPDATE`,
      [leagueId],
    );
    if (!lg[0]) throw new FantasyError('NOT_FOUND', 'League not found');
    ...
    if (!top || top.status !== 'archived') {
      throw new FantasyError(
        'CONFLICT',
        'Current season is not complete; finish the playoffs first',
      );
    }
    ...
    const { rows: created } = await client.query<SeasonRow>(
      `INSERT INTO fs_season
         (league_id, season_number, status, regular_weeks, playoff_seeds)
       VALUES ($1, $2, 'regular', $3, $4)
       RETURNING ${SEASON_COLS}`,
      [
        leagueId,
        nextNumber,
        lg[0].season_length_weeks,
        defaultSeeds(lg[0].size),
      ],
    );
    ...
    // lineups/scores/matchups are left intact — they carry the old number.
